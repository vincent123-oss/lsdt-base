...
def get_d4rl_normalized_score(score, env_name,index=None):
    if index:

        env_key = env_name.split('-v1')[0].lower()
    else:
...
def evaluate_on_env(model, device, context_len, env, rtg_target, rtg_scale,
                    num_eval_ep=10, max_test_ep_len=1000,goalconcate=False,
    ...
    model.eval()
    print("After calling model.eval(), model training mode:", model.training)
        ...
        for _ in range(num_eval_ep):
            ...
            for t in range(max_test_ep_len):
                ...
                if goalconcate:
                    ...
                    if index :
                        goal=env.get_target()#Maze2d
                    else:
                        goal=env.target_goal#Antmaze
                    goal_array = np.array(goal)
                    running_state1  = np.concatenate([running_state, goal_array])
                else:
                    running_state1=running_state
                total_timesteps += 1
               
                # add state in placeholder and normalize

                states[0, t] = torch.from_numpy(running_state1).to(device)
...
class D4RLTrajectoryDataset(Dataset):
    def __init__(self, dataset_path, context_len, rtg_scale):
        ...
        for traj in self.trajectories:
            ...
            traj['returns_to_go'] = discount_cumsum(traj['rewards'], 1) / rtg_scale
